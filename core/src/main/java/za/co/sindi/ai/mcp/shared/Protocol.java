...
public abstract class Protocol<T extends Transport, REQ extends Request, N extends Notification, RES extends Result> implements AutoCloseable {
	...
	/** Map of request handlers keyed by method name */
	private final ConcurrentHashMap<RequestId, CompletableFuture<?>> requestHandlersCancellations = new ConcurrentHashMap<>();
	...
	protected Protocol() {
		...
		addNotificationHandler(CancelledNotification.METHOD_NOTIFICATION_CANCELLED, notification -> onCancel(MCPSchema.toNotification(notification)));
	...
	public <T extends Result> CompletableFuture<T> sendRequest(REQ request, Class<T> resultType, final ProgressHandler progressHandler) {
		...
		Consumer<Throwable> cancel = throwable -> {
	        ...
	        transport.send(MCPSchema.toJSONRPCNotification(new CancelledNotification(parameters)));
...
//	        cf.completeExceptionally(throwable);
	...
	private void onClose() {
		final String type = this.getClass().getSimpleName(); //this instanceof Server ? "Server" : this instanceof Client ? "Client" : "Protocol";
	...
	private void onRequest(JSONRPCRequest request) {
		...
		if (handler == null) {
			...
			.exceptionallyCompose(throwable -> {
				LOGGER.log(Level.SEVERE, "Error handling request: " + request.getMethod() + ", ID: " + request.getId(), throwable);
				Throwable rootCause = Throwables.getRootCause(throwable);
				Protocol.this.onError(rootCause);
				if (rootCause instanceof MCPError error) {
					return sendError(request.getId(), error.getCode(), error.getMessage(), error.getData());
				} else {
				...
				requestHandlersCancellations.remove(request.getId());
			});
			requestHandlersCancellations.put(request.getId(), cf);
		}
	}
	
	private void onCancel(CancelledNotification notification) {
		if (notification.getParameters() == null || notification.getParameters().getRequestId() == null) return ;
		CompletableFuture<?> future = requestHandlersCancellations.get(notification.getParameters().getRequestId());
		if (future != null) {
