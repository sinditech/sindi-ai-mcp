...
public final class CreateMessageRequest extends BaseRequest implements ParameterBasedRequest<CreateMessageRequestParameters>, ServerRequest {
	...
	public static final class CreateMessageRequestParameters extends TaskAugmentedRequestParameters {
	...
	public static enum ToolChoice {
		 ...
		 * @param choice
		 */
		private ToolChoice(final String choice) {
			this.choice = choice;
		}
		
		public static ToolChoice of(final String value) {
			for (ToolChoice choice : values()) {
				if (choice.choice.equals(value)) return choice;
			}
			
			throw new IllegalArgumentException("Unrecognised tool choice '" + value + "'.");
		}
