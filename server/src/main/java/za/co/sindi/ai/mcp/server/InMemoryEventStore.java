...
public class InMemoryEventStore implements EventStore {
	...
	@Override
	public EventId storeEvent(StreamId streamId, JSONRPCMessage message) {
	...
	@Override
	public StreamId replayEventAfter(EventId lastEventId, Sender sender) {
		// TODO Auto-generated method stub
		...
		synchronized (events) {
			events.entrySet().stream().filter(entry -> entry.getValue().streamId().equals(streamId)).forEach(entry -> {
				if (entry.getKey().equals(lastEventId)) {
					foundLastEvent.set(true);
				} else {
					if (foundLastEvent.get()) {
						sender.send(entry.getKey(), entry.getValue().message);
					}
				}
			});
